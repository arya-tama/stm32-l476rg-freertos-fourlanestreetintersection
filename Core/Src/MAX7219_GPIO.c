...
/*
	******************************************************************************
	* @brief	(description ): Draws a scrolling text
	* @param	( parameters ): reference to an array with characters (the string itself)
                                X and Y coordinates (start of string rendering)
	* @return   (    returns ):
	******************************************************************************
*/ 
		...
		if ( (uint8_t)*text >= 0xC0 ){	// код 0xC0 соответствует символу кириллица 'A' по ASCII Win-1251
			...
			switch ((uint8_t)*text) {
				case 0xD0: {
					...
					else if ((uint8_t)*text == 0x81) { c = 0xA8; break; }		// байт символа Ё ( если нужнф еще символы добавляем тут и в функции DrawChar() )
